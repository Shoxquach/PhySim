...
namespace VCX::Labs::RigidBody {
    void CaseFourRigids::reset_simulation() {
    ...
    void CaseFourRigids::OnSetupPropsUI() {
        ...
        ImGui::Button("Next Frame", ImVec2(250, 0));
        if (ImGui::IsItemClicked()) {
            _nextFrame = true;
        }
        ...
        ImGui::SliderFloat("Mu", &_mu, 0.f, 5.f);
    ...
    Common::CaseRenderResult CaseFourRigids::OnRender(std::pair<std::uint32_t, std::uint32_t> const desiredSize) {
        ...
        if (!_pause || _nextFrame) {
            _nextFrame = false;
            for (int i = 0; i < _numRigids; i ++) {
                for (int j = i + 1; j < _numRigids; j ++) {
                    collide_test(_rigids[i], _dims[i], _rigids[j], _dims[j], _restitution, _correction_factor, _mu);
                }
                collide_test(_rigids[i], _dims[i], _ground, glm::vec3(_ground_size * 2.f, 0.f, _ground_size * 2.f), _restitution, _correction_factor, _mu);
                collide_test(_rigids[i], _dims[i], _walls[0], glm::vec3(_ground_size * 2.f, _wall_height, 0.f), _restitution, _correction_factor, _mu);
                collide_test(_rigids[i], _dims[i], _walls[1], glm::vec3(_ground_size * 2.f, _wall_height, 0.f), _restitution, _correction_factor, _mu);
                collide_test(_rigids[i], _dims[i], _walls[2], glm::vec3(0.f, _wall_height, _ground_size * 2.f), _restitution, _correction_factor, _mu);
                collide_test(_rigids[i], _dims[i], _walls[3], glm::vec3(0.f, _wall_height, _ground_size * 2.f), _restitution, _correction_factor, _mu);
            }
            ...
            // dims_list.push_back(glm::vec3(0.f, _wall_height, _ground_size * 2.f));
            // collide_multiple(rigid_list, dims_list, _restitution, _correction_factor, _mu, _pause);
            ...
            for (int i = 0; i < _numRigids; i++) {
                if(i != _selected) {
                    _rigids[i].SimulateTimestep(_timestep, _gravity * _rigids[i].m, glm::vec3(0.f));
                } else {
                    _rigids[i].SimulateTimestep(_timestep, force + _gravity, torque);
                }
            }
        }
    ...
    void CaseFourRigids::OnProcessInput(ImVec2 const & pos) {
        // printf("Mouse Pos: %f, %f\n", pos.x, pos.y);
        _cameraManager.ProcessInput(_camera, pos);
    }

    void CaseFourRigids::OnProcessMouseControl(glm::vec3 mouseDelta) {
